  ...
  def self.print(property)
    ...
    property.each do |val|
      ...
      when 0x84
        ...
        ret << "MeasuredInstantaneousPowerConsumption.value #{total}"
      else
        ## nothing
      end ## end of case
...
if ARGV[0] == "config"
  puts "graph_title AirCleaner Instantaneous Power Consumption monitor"
  puts "graph_args --base 1000 -l 0"
  puts "graph_vlabel values"
  puts "graph_category life"
  puts "graph_info AirCleaner monitor by echonet."
  puts "MeasuredInstantaneousPowerConsumption.label Measured Instantaneous Power Consumption kWh"
  puts "MeasuredInstantaneousPowerConsumption.type GAUGE"
  exit 0
end


aircon_eoj = BEOJ.new
aircon_eoj.set_values 0x01,0x35,0x01
